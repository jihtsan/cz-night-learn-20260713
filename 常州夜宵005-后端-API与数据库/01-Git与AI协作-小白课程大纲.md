...
对零基础学员，不建议一上来就背命令。先让他们看见“一个版本节点怎样出现、怎样上线、怎样分叉”，再把命令贴到图上。

### 图解 1：一次改动的完整旅程

```mermaid
flowchart LR
    A[工作区<br/>正在改文件] -->|git add| B[暂存区<br/>选中本次改动]
    B -->|git commit| C[本地仓库<br/>时间线新增节点]
    C -->|git push| D[远程仓库<br/>在线分支同步]
```
...
| 工作区 | 厨房操作台 | 我刚改了哪些东西？ |
| 暂存区 | 打包台 | 这次准备把哪些变化装箱？ |
| 本地仓库 | 自家档案室 | 哪些改动已经正式存档？ |
| 远程仓库 | 云端总仓库 | 哪些版本团队已经能看到？ |

### 图解 2：Commit 是给本地时间线增加一个节点

```mermaid
flowchart LR
    C1((C1<br/>初始页面)) --> C2((C2<br/>增加按钮))
    C2 -->|git commit| C3((C3<br/>修复登录))
    W[已暂存的改动] --> C3
```

读图方法：

- 圆点不是一个单独文件，而是项目在那一刻的存档节点；
- C1 → C2 → C3 是一条本地版本时间线；
- `git commit` 只会新增本地节点 C3，此时 GitHub 还不知道 C3 的存在。

> [!tip] 一句话记忆
> Commit 像游戏里按下“保存进度”，存档先留在自己电脑上。

### 图解 3：Push 把本地新节点同步到在线分支

```mermaid
flowchart LR
    subgraph L[本地 feature/login 分支]
        direction LR
        L1((C1)) --> L2((C2)) --> L3((C3 新))
    end
    subgraph R[GitHub 上的 origin/feature/login]
        direction LR
        R1((C1)) --> R2((C2)) --> R3((C3<br/>Push 后新增))
    end
    L3 ==>|git push| R3
```
...
1. Push 前，本地已经有 C3，在线分支还停在 C2；
2. `git push` 把缺少的 C3 传到对应的远程分支；
3. Push 后，本地和在线分支都指向 C3。

> [!important] Commit 不等于 Push
> Commit 是本地存档，Push 才是上传同步。拍了照，不等于已经发朋友圈。

### 图解 4：Branch 从已有 Commit 拉出一条新时间线
...
- `main` 和 `feature-login` 都从 C2 开始，但之后可以各自向前；
- Branch 不是复制一份“项目-副本”，而是在同一棵历史树上拉出新线；
- AI 可以在 `feature-login` 上试验，不需要立刻影响 `main`；
- 试验成功后可以再讲 Merge 或 Pull Request，本节不展开冲突细节。

### 补充概念图：Worktree 是给不同分支配独立工作台

```mermaid
flowchart TB
    R[同一个 Git 仓库] --> A[project 文件夹<br/>main 分支<br/>你在工作]
    R --> B[project-ai 文件夹<br/>feature/ai-tests 分支<br/>AI 在工作]
```

Worktree 只讲三句话：

1. Branch 是不同时间线，Worktree 是不同文件夹；
2. 它让人和 AI 可以同时在不同分支工作；
3. 小白知道 `git worktree list` 可以查看工作树就足够了，不要在本节讲删除、修复和锁定。

### 哪些内容适合画图

| 内容 | 最合适的形式 | 原因 |
|---|---|---|
| `add → commit → push` | 流程图 | 它是一组有先后顺序的动作 |
| Commit | 时间轴 | 便于看见“又多了一个存档节点” |
| Push | 本地 / 远程双轨图 | 便于对比上传前后两条分支是否同步 |
| Branch | 树状分叉图 | 直接表现从某个 Commit 拉出新时间线 |
| Worktree | 两个工作台示意图 | 只需建立“一个仓库、多个目录”的概念 |
| `status`、`diff`、`log` | 命令卡 + 现场演示 | 它们的价值在实际输出，画静态图反而不够直观 |
...
| 多工作区 | 怎样查看当前有哪些工作区？ | `git worktree list` |
...
| 12‑24 分钟 | 一次改动的旅程 | 工作区、暂存区、Commit、Push | 先读流程图，再改一行文字 |
| 24‑34 分钟 | 平行宇宙 Branch | 建分支、切分支、合并概念 | 看树状图，同一页做两种配色 |
| 34‑37 分钟 | Worktree 知道即可 | 一仓库、多分支、多文件夹 | 只看概念图，不要求实操 |
| 37‑47 分钟 | 与 GitHub 同步 | Clone、Fetch、Pull、Push、PR | 用双轨图和快递比喻讲解 |
| 47‑55 分钟 | AI 协作安全流程 | 先看状态、再看差异、小步提交、审查后合并 | 命令卡 + AI 对话卡 |
| 55‑60 分钟 | 挑战任务 | 学员完成一次安全改动 | 两人一组实操 + 复盘 |

> [!note] 45 分钟压缩方案
> Worktree 只保留 2 分钟概念图，不让学员动手；Fetch 和 Pull 的底层关系放入课后延伸。
...
课堂上只对照前面的 Worktree 概念图看输出：每一行是一个工作目录，后面标出它正在使用的分支。新建、删除和修复 Worktree 都留到进阶课。
...
| 你想做什么 | 可以直接对 AI 说 |
|---|---|
| 看现在的状态 | “只检查 Git 状态，告诉我当前分支和未提交改动，不要修改。” |
| 开始一项新任务 | “先为这个任务创建新分支，不要直接改 main。” |
| 检查 AI 改了什么 | “请总结当前 diff，按文件说明改动和风险。” |
| 准备提交 | “只暂存与本任务有关的文件，列出后先等我确认。” |
| 准备上传 | “推送前再检查状态和测试结果，不要强制 push。” |
